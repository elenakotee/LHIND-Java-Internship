...
public class OrderController {
	...
	public ResponseEntity<List<Order>> findAllOrdersForRestaurant(@RequestBody RequestRestaurant restaurant) {
		logger.debug("restaurant manager listing all their restaurant's orders \n");
		List<Order> result = orderService.retrieveByRestaurant(restaurant);
	...
	public ResponseEntity<List<Order>> findByOrderStatus(@PathVariable("status") String status,
														 @RequestBody RequestRestaurant restaurant) {
		logger.debug("restaurant manager lfiltering orders by status \n");
		List<Order> result = orderService.retrieveByRestaurantAndStatus(restaurant, status);
		return ResponseEntity.ok(result);
	}

	@PostMapping("/makeorder")
	@PreAuthorize("hasRole('CLIENT')")
	public ResponseEntity<Order> makeOrder(@RequestBody List<Integer> items, @RequestParam(name = "id") Integer id) {
		logger.debug("a client making an order \n");
		return new ResponseEntity<>(orderService.makeOrder(items, id),HttpStatus.CREATED);
	}
